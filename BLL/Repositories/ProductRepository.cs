...
namespace BLL.Repositories
{
    public class ProductRepository : IProductRepository
    {
        ...
        public async Task<List<Product>> GetSalesProduct()
        {
            return await context.Products.Where(p => p.Discout > 0).ToListAsync();
        }

        public async Task<List<string>> GetProductsName()
        {
            return await context.Products.Select(s => s.Name).ToListAsync();
        }
    }
}
